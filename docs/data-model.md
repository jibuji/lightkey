...
| 规则 rule | `{uuid}.rule.lk` | K_data | 授权门规则（M2：inject 白名单；M2.9 增 `capability=read` 读规则，schema 见 value-disclosure.md §4；**M2.97 规划增 `capability=write` + `actions` 写规则**，schema 见 [write-gate.md](write-gate.md) §4） |
