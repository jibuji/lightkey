...
| [architecture-deepening.md](architecture-deepening.md) | 架构深化计划（授权门管线收敛 + 假设缝移除；四项行为保持重构 + 回归边界；唯一出处，拍板 #28） | 架构维护 |
| [quick-capture.md](quick-capture.md) | 快速保存（唯一实现规格，补充拍板 #27，issue #161，已实现）：托盘一键把剪贴板 secret 存进 LightKey——值预填、命名即存；复用 item.put 桌面豁免、不做剪贴板监听、锁态引导解锁 | M2.99 |
...
| [../workflows/issue-autopilot.md](../workflows/issue-autopilot.md) | **workflow 规格**（唯一出处，补充拍板 #29）：GitHub issue 无人值守分诊→认领→实现→开 PR→CI 绿自动合并；能力词表与跳账、NEEDS-HUMAN 协议、denylist 护栏、心跳与两层看活 | 工程效率 |
| [reviews/2026-09-09-issue-autopilot.md](reviews/2026-09-09-issue-autopilot.md) | issue-autopilot 机制评审 + 整改计划（A1–A19 / Phase 0–4；配套补充拍板 #31–#33） | 工程效率 |
