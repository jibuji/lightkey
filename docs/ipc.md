...
| `item.put` / `item.delete` | 写（**写门已实现 M2.97**：socket 通道走三层——写规则命中静默、未命中弹窗、headless `authz.denied`（-32017 复用）；update 双向名称约束、**delete 恒弹窗任何规则不豁免**；desktop 直调受信豁免；锁态 `session.invalid` 先行；RPC 不拆，action 由 daemon 从 `ItemPutParams.id` 有无权威派生；审计 command 派生 `item.create/update/delete <name>`。完整规格见 [write-gate.md](write-gate.md)） | 新 revision |
