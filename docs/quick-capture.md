...
- 状态：**草案（Draft，待拍板）**——2026-09-07 起草，关联 issue #161。本文是
  拟议实施方案规格；获拍板后按惯例在 decisions.md 记决策、翻转本文状态为
  **已拍板**再按 PR 序列落地。拍板前任何决策点（§9）不得擅改。
...
1. **锁态一步式「主密码 + 保存」**：默认**不做**（与 write-gate.md §12
   留档一致）；本规格锁态只引导解锁 + pending flush。
2. **pending 生命周期**：锁定不清、面板消费/解锁消费后清除（初版）；
   是否「锁定即清」由拍板定。
3. **名称启发建议**：内置保守前缀表（§4.3）且设置页可关（默认开）。
4. **入口落审计**：默认不记；如需「快速保存使用次数」类指标再议
   （当前审计模型按数据变更事件，不记 UI 入口）。
5. **里程碑归属**：拟议 **M2.99（快速保存）**，插入 M2.98 之后、M3 之前
   （M3 标签保持浏览器填充不动，与历次补充分批同惯例）；以拍板为准。
