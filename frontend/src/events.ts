...
export interface AuthzRequestPayload {
  ...
  needsUnlock: boolean;
  /** 审批类型（M2.9 值披露 + 补充拍板 #22 规则门 + M2.97 写门 #24）：弹窗
   *  按 kind 选形态；缺省视为未知（防御渲染，不回退 inject——协议演进时
   *  旧 UI 不误导）。字面量与 `ipc/protocol.ts` 的 `APPROVAL_KINDS`（镜像
   *  Rust `ApprovalKind` serde 值）一致。 */
